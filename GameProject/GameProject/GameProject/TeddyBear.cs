    ...
    public class TeddyBear
    {
        ...
        public void Update(GameTime gameTime)
        {
            ...
            if (elapsedShotTime > firingDelay)
            {
                elapsedShotTime = 0;
                Game1.AddProjectile(
                           new Projectile(ProjectileType.TeddyBear, Game1.GetProjectileSprite(ProjectileType.TeddyBear),
                               X, Y + GameConstants.FRENCH_FRIES_PROJECTILE_OFFSET, -GameConstants.TEDDY_BEAR_PROJECTILE_SPEED));
                shootSound.Play(0.2f,0.0f,0.0f);
            }
            else { elapsedShotTime += gameTime.ElapsedGameTime.Milliseconds; }
